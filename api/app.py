...
# 'auffuellung'/'entnahme'/'delta' are vorrat/stock bookkeeping, not a "moment" worth
# showing in a life feed (see tracking.bensn.me v1.6.0's habit-tracker-focus redesign).

def fetch_tracking_events(limit=500):
    """Fetch recent consumption entries from tracking.bensn.me and convert to feed events."""
    if not DB_URL:
        return []
    try:
        conn = get_db()
        cur = conn.cursor()
        cur.execute('''
            SELECT id, timestamp, category, name, amount, unit, note
            FROM tracking_entries
            WHERE entry_type = 'zaehler' AND (deleted IS NULL OR deleted = FALSE)
    ...
    return [{
        'id': f'tracking-{r["id"]}',
        'type': 'tracking_logged',
        'date': utc_to_vienna_naive(r['timestamp'].isoformat()),
        'category': r['category'],
        'name': r['name'],
