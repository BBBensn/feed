...
def fetch_shift_events(limit=100, include_details=True):
    ...
    try:
        r = _req.urlopen(f'http://localhost:5001/api/shifts?limit={limit}', timeout=5)
        shifts = json.loads(r.read().decode())
    ...
    for shift in shifts:
        ...
        breaks = []
        try:
            rb = _req.urlopen(f'http://localhost:5001/api/shift/{shift["id"]}', timeout=5)
            detail = json.loads(rb.read().decode())
